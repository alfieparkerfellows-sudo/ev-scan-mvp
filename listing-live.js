(() => {
  ...
  style.textContent = `
    ...
    .live-score-row{display:grid;grid-template-columns:1fr;gap:12px;margin-top:18px}.live-score{border-radius:18px;background:rgba(255,255,255,.04);padding:16px}.live-score b{display:block;font-size:1.8rem}.live-score small{color:var(--muted,#9ba7ba)}
    .live-verdict{margin-top:18px;font-size:1.05rem;line-height:1.6;color:#e9edf5}
    .live-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-top:18px}
    ...
    .live-evidence{display:flex;gap:8px;flex-wrap:wrap}.live-chip{border:1px solid rgba(102,226,160,.22);background:rgba(102,226,160,.08);color:#77e6aa;border-radius:999px;padding:7px 10px;font-size:.7rem;font-weight:850}.live-chip.estimate{border-color:rgba(255,196,89,.24);background:rgba(255,196,89,.08);color:#ffc459}
    ...
    .live-note{padding:14px 16px;border-radius:16px;background:rgba(255,196,89,.06);border:1px solid rgba(255,196,89,.18);color:#d9c9a7;line-height:1.55;font-size:.82rem}
    @media(max-width:850px){.live-report-grid{grid-template-columns:1fr}.live-metrics{grid-template-columns:repeat(2,1fr)}}
    @media(max-width:560px){.live-report{padding:18px 14px 55px}.live-panel{padding:17px;border-radius:18px}.live-metrics{grid-template-columns:1fr}.live-hero-photo,.live-hero-photo img,.live-hero-photo .live-photo-fallback{min-height:245px}}
  `;
  ...
  function showOverlay() {
    ...
    if (copy) copy.textContent = 'Cross-checking the listing against available vehicle and official MOT data.';
    ...
    report.innerHTML = `<main class="live-report">
      ...
      <section class="live-report-grid">
        <article class="live-panel live-hero-photo">
          ...
          ${image ? `<img src="${escapeHtml(image)}" alt="${escapeHtml(payload.vehicleName || 'Scanned EV')}">` : '<div class="live-photo-fallback">Vehicle photo verified from the advert source.</div>'}
        </article>
        <article class="live-panel live-summary">
          ...
          <div class="live-price"><div><span>Advert asking price</span><strong>${money(listing.price)}</strong></div><div><span>Market comparison</span><strong>Not claimed</strong></div></div>
          <div class="live-score-row"><div class="live-score"><small>Decision confidence</small><b>${escapeHtml(confidence.score ?? '—')}%</b><small>Released only after the strict evidence gate</small></div></div>
          <p class="live-verdict">${escapeHtml(payload.verdict || '')}</p>
        </article>
      </section>

      <section class="live-metrics">
        <article class="live-panel live-metric"><h3>Battery specification</h3><strong>${escapeHtml(battery.capacityKwh)} kWh</strong><p>Advert/provider-backed specification. Battery State of Health is not guessed.</p></article>
        <article class="live-panel live-metric"><h3>EV range specification</h3><strong>${number(battery.ratedOrListedRangeMiles)} mi</strong><p>Listed/rated figure from the available vehicle data, not a fabricated real-world result.</p></article>
      ...
      <section class="live-report-grid live-section">
        <article class="live-panel">
          ...
          <div class="live-evidence">${providers}<span class="live-chip">DVSA verified</span><span class="live-chip estimate">No unverified market score</span><span class="live-chip estimate">Battery SoH not measured</span></div>
          <p>EV Scan matched the advert to a UK registration and verified the vehicle/MOT identity before showing this page. It has deliberately left out market-value claims because there is not yet a permanent zero-cost independent market source we are willing to rely on.</p>
          <p class="live-note">${escapeHtml(battery.note || '')}</p>
        </article>
        <article class="live-panel"><h2>Questions worth asking the seller</h2><ul class="live-list">${questionList(payload.sellerQuestions)}</ul></article>
      </section>
  ...
  async function runLiveScan(value) {
    ...
    const help = $('#url-help');
    if (help) { help.classList.remove('error'); help.textContent = 'Checking the listing against available verification sources…'; }
    showOverlay();
    try {
